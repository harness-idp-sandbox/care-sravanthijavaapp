...
# run app
java -jar target/sravanthijavaapp-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/sravanthijavaapp:latest   --push .
```
...
```bash
# namespace (templated)
kubectl get ns care >/dev/null 2>&1 || kubectl create ns care
...
# watch rollout
kubectl rollout status deploy/sravanthijavaapp -n care

# get external endpoint
kubectl get svc sravanthijavaapp-svc -n care
```
...
```bash
kubectl port-forward svc/sravanthijavaapp-svc 8080:80 -n care
...
kubectl rollout restart deploy/sravanthijavaapp -n care
kubectl rollout status deploy/sravanthijavaapp -n care
```
...
info.app.name=sravanthijavaapp
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/sravanthijavaapp/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/sravanthijavaapp/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© sravanthi.gottam@carelumen.com. For demo/PoV purposes.
